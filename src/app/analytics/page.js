...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    async function loadMetrics() {
      try {
        ...
        setError("Failed to load metrics. Ensure metrics.json exists in backend.");
      } finally {
        setLoading(false);
      }
    }
  ...
  if (error || !metrics) {
    return (
      <div className="flex items-center justify-center min-h-[60vh] text-[var(--color-danger)]">
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 min-h-screen">
      ...
      >
        <h1 className="text-4xl font-extrabold mb-4">Model Performance Analytics</h1>
        <p className="text-[var(--color-text-muted)] max-w-2xl mx-auto">
          Comprehensive evaluation metrics for all 9 trained models across the three distinct classification tasks.
        </p>
      </motion.div>
